...
public class Maa {
    ...
    /*
    1. Always look the worst case --> big-O notation
    2. We would consider large/infinte input size
    3. Ingore the less dominating terms or lower terms
     */
    static void func(int n) {
        for(int i=1; i<=n; i=i+2){  // times - n
            System.out.println("upGrade");  // --> O(1)
        }
    }
    // f(5) -> O(1) + O(1) +... 5 times -> 5*O(1) -> O(5) -> O(1)
    // f(50) -> O(1) + ... 50 times -> 50*O(1) -> O(50) constant time algo
    // f(n=10^6) -> O(1) + .... n times -> n*O(1) -> O(n) - linear time algo


    // 10^6 -> 10^6 --> 10^6 (half)-> 10^3
    // i++ --> i + 10^4
    ...
    static void func4(int n){
        for(int i=n ; i>0 ; i=i/2){
            System.out.println("Drop the Egg");
        }
    }
    // n is the no of floors in the building - 10
    // i = 10; 10 > 0 ; drop the egg; i = i/2 i.e i = 10/2 = 5
    // i = 5; 5 > 0; drop the egg; i = i/2 i.e. i = 5/2 = 2
    // i = 2; 2 > 0; drop the egg; i = i/2 i.e. i = 2/2 = 1
    // i = 1; 1 > 0; drop the egg; i = i/2 i.e. i = 1/2 = 0
    // n + n/2 + ((n/2)/2 = n/4 or n/2^2) + n/2^3 + ...
    // Itrerative Approximation of Log =
    // log2(n) = n + n/2 + ((n/2)/2 = n/4 or n/2^2) + n/2^3 + ...
